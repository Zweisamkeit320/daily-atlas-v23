...
v2.4.4 继承 v2.4.3 的弱网与首屏路径，不改变候选池、推荐逻辑、用户数据结构或备份格式。书、电影和城市在主卡、详情预览与探索结果中显示“同源开放许可图片”“第三方渐进图片”或“本地编辑视觉”；图片只有在完成解码且仍属于当前条目后才显示，全部候选失败会明确回到当前条目的本地编辑视觉。诊断页把关键同源故障与固定 CDN／设备语音后备等非必要降级分开。书封和海报没有逐项再分发许可时不复制到同源包，完整准入规则与维护证据见 [`docs/RELEASE_v2.4.4.md`](docs/RELEASE_v2.4.4.md) 和 [`docs/MAINTENANCE_v2.4.4.md`](docs/MAINTENANCE_v2.4.4.md)。
...
| HTTP／PWA 的 2,200 项紧凑选择 JSON | 79,148 B gzip（约 77.3 KiB） | 启动时；经字节数、SHA-256 与 SRI 校验后重建选择目录 |
| `file://` 的紧凑选择脚本回退 | 约 78.2 KiB gzip | 仅双击打开时使用；HTTP／PWA 不下载 |
| 当日五项详情分片 | 约 79.4 KiB gzip（当前日期示例） | 五个 ID 选出后，并发上限 2–4 |
| 延迟搜索索引 | 约 387.5 KiB gzip | 用户进入探索并实际查询时，由 Worker 加载 |
...
v2.4.4 默认按需显示第三方渐进书封／电影海报和已复核的同源城市风貌图。书影图片不在构建时批量下载或宣称商业再分发权；城市图均有 Commons 文件页、作者、开放许可和本地 SHA-256，完整署名见 [`city-credits.html`](city-credits.html)。失败、数据节省或安全模式下统一回到明确标注的本地编辑视觉。移动端优先请求 480×270 衍生图，宽屏优先使用 960×540 原发布图；两者均来自同一已复核文件。完整离线仍以 200 张原发布城市图为核验包，移动衍生图按需缓存，离线时可立即回退到已核验原图。
...
本项目的自动移动验证使用桌面 Edge/Chromium 和 320、360、390、428、768 px 视口；它能验证响应式布局、弹窗和一部分 Web 平台降级，但不能冒充实体 Android、iPhone、夸克或微信内置浏览器，也不能证明某个运营商网络可达。v2.4.4 的待执行真机矩阵、逐步操作和通过标准见 [`docs/REAL_DEVICE_MATRIX_v2.4.4.md`](docs/REAL_DEVICE_MATRIX_v2.4.4.md)。在实体设备记录结果前，状态必须保持 `NOT_RUN`。
...
本仓库不把任何历史 URL、平台控制台状态、候选 ZIP 或旧版本提交写成 v2.4.4 的上线结论。目标 Origin、实际响应头、资源逐项比对和实体设备矩阵必须在本版静态包生成后重新记录；在这些记录齐全前，状态仅为“待部署/待验收”。正式上传必须使用精简静态部署包，不要把包含 `data/upstream`、测试和构建脚本的完整审计包公开部署。
...
$staticZip = "$publishRoot\daily-atlas-static-v2.4.4-r1-$releaseStamp.zip"
...
5. 确认默认模式显示书封／电影海报，图片故障能恢复本地视觉；开启数据节省后远程图片请求归零，本周回顾仍只读本机记录。

每次发布前必须先运行 `npm run build:pwa` 和 `npm run check:pwa`。Service Worker 的壳、内容、医学图、搜索、音频和视觉六个版本由各自内容生成；远程书封／电影海报只作尽力缓存，而用户主动选择完整离线时，500 个固定音频和 200 张同源城市图须逐项通过字节／哈希验证后才计入完整离线包。若仍看到旧版：先关闭同一站点的其他标签页，再重新打开并刷新；不要一开始就清除站点数据，因为那会删除本机记录和偏好。确需清除前先导出备份，再优先使用设置页或诊断页的“修复应用缓存”。

域名或子域名改变会形成新的浏览器 Origin；旧地址的收藏、偏好、Cache 和安装状态不会自动迁移到新地址。应先从旧地址导出 JSON，再在新地址查看差异并选择“替换”或“合并”。不同浏览器和微信内置浏览器也各有独立站点存储。

最后按 [`docs/REAL_DEVICE_MATRIX_v2.4.4.md`](docs/REAL_DEVICE_MATRIX_v2.4.4.md) 用实体设备分别执行 Wi-Fi 与移动数据测试。若电脑能开而某个手机网络在收到任何 HTTP 响应前就超时，应检查该网络的 DNS、代理、运营商与域名可达性；`pages.dev`、`workers.dev` 和自定义域名在不同地区的实际连通性不能由应用代码保证。面向中国大陆稳定公开运营时，还需根据服务器所在地、域名备案和所选云厂商要求另行设计境内部署或镜像。
...
“探索 2,200 项”在同一页面提供延迟建立的本地索引和分页结果，默认每页 24 项，不会一次把全部内容写入手机 DOM。搜索支持中文、英文和德文归一化，并可按类型、书影题材／年代／评分比例、城市地区、德语等级、医学主题和排序方式组合筛选。评分筛选只比较各来源分数相对满分的比例，不把 Open Library 与 IMDb 混成一个综合分。
...
v2.4.4 的主卡与搜索结果共用视觉路由：书影按白名单第三方渐进加载，城市仅使用同源开放许可文件；数据节省／安全模式与图片故障均回到明确标注的本地编辑视觉，本地医学图始终保留。紧凑索引仍独立降低首屏目录传输；“完整离线”下载全库详情、搜索索引、500 条朗读和 200 张原发布城市图，但不把远程书封／海报冒充离线资产。移动城市衍生图按需缓存，完整离线时可立即使用已核验原图回退。
...
用户明确选择“完整离线”后，Service Worker 才把 44 个详情分片、延迟搜索索引、500 个朗读和 200 张同源城市图下载到独立暂存 Cache；进度为 `0–700`（音频 `1–500`、城市图 `501–700`）。每项写入前核对字节数和 SHA-256；完整 marker 还会复核真实 Cache 项。暂停、页面刷新、Worker 更新、网络或配额故障会保留已经验证的暂存，继续时只补缺项；“取消”才删除可选暂存。轻量应用壳始终保留。远程书封／电影海报只作在线渐进视觉和尽力缓存，不能作为完整包已验证资产；实时天气、Web Speech 系统语音包和外部来源页面不缓存。

Service Worker 分别计算应用壳、内容、医学图、搜索、音频和视觉六个版本。新 worker 会先写入发生变化的必要包，并复用内容版本未变且 marker／实际条目都通过核对的旧包；远程书影视觉的运行时缓存失败不阻断文字卡片，200 张同源城市图则在用户选择完整离线时进入逐项核验的视觉包。检测到等待中的完整更新后，设置页显示“应用更新并重新载入”，只有用户点击才切换。激活后清理不再属于当前版本的应用 Cache，避免新旧壳静默混用。

首次轻量安装仍需联网完成一次；用户选择完整离线后，500 个音频、200 张城市图和完整目录当前估算约需新增 36 MB（约 35.76 MiB），并需预留应用壳、医学图及浏览器缓存开销。设置页会显示浏览器估算的已用、配额、可用空间与持久存储状态；可由用户手势申请持久存储，也可在不删除个人数据的前提下核对／修复应用 Cache。安装提示是否出现由浏览器决定；`file://`、局域网普通 HTTP、隐私模式、企业策略或浏览器存储限制可能禁用 PWA。缓存应用壳不等于拥有第三方封面、海报或元数据的再分发权。
...
- 书封是远程资源，其许可与书目元数据必须分别判断。

### 电影

- IMDb title ID；评分至少 `7.5 / 10`，票数至少 `30,000`；
- 数值逐条对应随包上游审计快照中的 IMDb 非商业 `title.ratings.tsv.gz` 记录；
- 热度层只表示 IMDb 票数：`classic > 500,000`、`mid = 100,001–500,000`、`underseen <= 100,000`，且所有条目仍满足 30,000 票底线；
- 题材、导演、片长和简介筛查参考 Cinemeta 快照；页面不复制 Cinemeta 的长简介；海报为远程资源。

IMDb 数据的使用受其个人、非商业数据条款约束。公开运营或商业化前，必须重新解决评分、书目、封面、海报和其他元数据的许可，不应把本审计快照当作生产授权。
...
```powershell
npm ci
# v2.4.4 跨引擎门禁需要三套 Playwright 浏览器
...
```powershell
npm run release:preflight
npm run release:package -- --zip "C:\Users\lenovo\Documents\今日万象发布\daily-duet-v2.4.4-r1-YYYYMMDD-HHMMSS.zip"
npm run release:verify -- --zip "C:\Users\lenovo\Documents\今日万象发布\daily-duet-v2.4.4-r1-YYYYMMDD-HHMMSS.zip"
```
