...
v2.4.3 继承 v2.4.2 的可靠性收口，不改变候选池、推荐逻辑或用户数据结构。正常启动将 14 个页面模块构建为 2 个确定性运行包；GitHub Pages 的不可变目录分片在同源连接迟迟未完成时，延迟 1.8 秒启动经固定提交和 SHA-256 校验的 CDN 竞速，先通过完整性验证者胜出。图片在解码成功前保持隐藏，第三方图源连续失败会触发 60 秒会话级熔断；200 张城市图新增 480×270 同源移动衍生图，字节总量约为原 960×540 图的四分之一。诊断页记录最近一次首页各阶段耗时，专门的受控弱网门禁检查首屏请求数、图片身份、移动裁切和换项响应。
...
v2.4.3 默认按需显示书封、电影海报和已复核的城市风貌图。书影图片只远程展示，不在构建时批量下载或宣称商业再分发权；城市图均有 Commons 文件页、作者、开放许可和本地 SHA-256，完整署名见 [`city-credits.html`](city-credits.html)。失败、数据节省或安全模式下统一回到程序化视觉。移动端优先请求 480×270 衍生图，宽屏优先使用 960×540 原发布图；两者均来自同一已复核文件。完整离线仍以 200 张原发布城市图为核验包，移动衍生图按需缓存，离线时可立即回退到已核验原图。
...
本项目的自动移动验证使用桌面 Edge/Chromium 和 320、360、390、428、768 px 视口；它能验证响应式布局、弹窗和一部分 Web 平台降级，但不能冒充实体 Android、iPhone、夸克或微信内置浏览器，也不能证明某个运营商网络可达。v2.4.3 的待执行真机矩阵、逐步操作和通过标准见 [`docs/REAL_DEVICE_MATRIX_v2.4.3.md`](docs/REAL_DEVICE_MATRIX_v2.4.3.md)。在实体设备记录结果前，状态必须保持 `NOT_RUN`。
...
本仓库不把任何历史 URL、平台控制台状态、候选 ZIP 或旧版本提交写成 v2.4.3 的上线结论。目标 Origin、实际响应头、资源逐项比对和实体设备矩阵必须在本版静态包生成后重新记录；在这些记录齐全前，状态仅为“待部署/待验收”。正式上传必须使用精简静态部署包，不要把包含 `data/upstream`、测试和构建脚本的完整审计包公开部署。
...
$staticZip = "$publishRoot\daily-atlas-static-v2.4.3-r1-$releaseStamp.zip"
...
最后按 [`docs/REAL_DEVICE_MATRIX_v2.4.3.md`](docs/REAL_DEVICE_MATRIX_v2.4.3.md) 用实体设备分别执行 Wi-Fi 与移动数据测试。若电脑能开而某个手机网络在收到任何 HTTP 响应前就超时，应检查该网络的 DNS、代理、运营商与域名可达性；`pages.dev`、`workers.dev` 和自定义域名在不同地区的实际连通性不能由应用代码保证。面向中国大陆稳定公开运营时，还需根据服务器所在地、域名备案和所选云厂商要求另行设计境内部署或镜像。
...
v2.4.3 的主卡与搜索结果共用视觉路由：书影按白名单远程渐进加载，城市仅使用同源开放许可文件；数据节省／安全模式与图片故障均回到本地程序化视觉，本地医学图始终保留。紧凑索引仍独立降低首屏目录传输；“完整离线”下载全库详情、搜索索引、500 条朗读和 200 张原发布城市图，但不把远程书封／海报冒充离线资产。移动城市衍生图按需缓存，完整离线时可立即使用已核验原图回退。
...
```powershell
npm ci
# v2.4.3 跨引擎门禁需要三套 Playwright 浏览器
...
```powershell
npm run release:preflight
npm run release:package -- --zip "C:\Users\lenovo\Documents\今日万象发布\daily-duet-v2.4.3-r1-YYYYMMDD-HHMMSS.zip"
npm run release:verify -- --zip "C:\Users\lenovo\Documents\今日万象发布\daily-duet-v2.4.3-r1-YYYYMMDD-HHMMSS.zip"
```
