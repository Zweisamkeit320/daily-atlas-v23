# 今日万象 v2.6.0 LTS Visual Edition：原图优先与原创主题插画回退版
...
v2.6.0 LTS Visual Edition 继承 v2.5.0 已完成的弱网、移动视觉和长期稳定路径，不改变候选池、推荐逻辑、用户数据结构或备份格式。公开版的 500 本书与 500 部电影默认优先在线加载条目已有的原书封／电影海报；图片缺失、失败、离线、数据节省或安全模式下，主卡、详情预览和探索结果均显示按作品类型、题材和稳定 ID 确定生成的原创本地 SVG 主题插画。第三方书影图片不复制进发布包或应用缓存。城市继续显示逐项复核、同源发布的开放许可风貌图；完成解码且仍属于当前条目后才显示，失败会回到当前条目的本地编辑视觉。诊断页继续显示 LTS 与“功能冻结，仅维护修复”。

本版本是经用户明确授权增加原创插画后的视觉功能基线。发布后只处理可复现核心故障、安全／兼容性、来源许可、危险内容与可访问性阻断，不再持续扩充功能或内容规模。完整范围见 [`docs/LTS_POLICY_v2.6.0.md`](docs/LTS_POLICY_v2.6.0.md)，本次发布证据见 [`docs/RELEASE_v2.6.0.md`](docs/RELEASE_v2.6.0.md)；v2.5.0 的维护证据继续作为未改功能的继承基线。项目自身使用许可和第三方通知见 [`LICENSE.txt`](LICENSE.txt) 与 [`NOTICE.txt`](NOTICE.txt)。
...
v2.6.0 默认按作品 ID 向严格白名单图片服务渐进请求原书封／电影海报，加载并解码成功后优先显示；请求可能向图片服务暴露当前作品 ID，但不携带收藏、偏好或探索记录。失败时立即保留同一条目的原创本地主题插画；数据节省和显式安全模式会完全停止第三方书影图片请求。第三方图片采用 network-only 策略，不进入应用 Cache 或离线包。城市图均有 Commons 文件页、作者、开放许可和本地 SHA-256，完整署名见 [`city-credits.html`](city-credits.html)。城市图片失败、数据节省或安全模式下会回到明确标注的本地编辑视觉。移动端优先请求 480×270 衍生图，宽屏优先使用 960×540 原发布图；两者均来自同一已复核文件。完整离线仍以 200 张原发布城市图为核验包，移动衍生图按需缓存，离线时可立即回退到已核验原图。
...
本项目的自动移动验证使用桌面 Edge/Chromium 和 320、360、390、428、768 px 视口；它能验证响应式布局、弹窗和一部分 Web 平台降级，但不能冒充实体 Android、iPhone、夸克或微信内置浏览器，也不能证明某个运营商网络可达。v2.6.0 的安卓主路径已由用户在真实安卓手机上确认通过：夸克、vivo 系统浏览器、微信内置浏览器的 Wi-Fi／移动数据六格均为 `PASS`。证据来源、验收口径与未具备设备的 `BLOCKED` 项见 [`docs/REAL_DEVICE_MATRIX_v2.6.0.md`](docs/REAL_DEVICE_MATRIX_v2.6.0.md)。
...
本仓库不把任何历史 URL、平台控制台状态、候选 ZIP 或旧版本提交写成 v2.6.0 的上线结论。目标 Origin、实际响应头、资源逐项比对和实体设备矩阵必须在本版静态包生成后重新记录；在这些记录齐全前，状态仅为“待部署/待验收”。正式上传必须使用精简静态部署包，不要把包含 `data/upstream`、测试和构建脚本的完整审计包公开部署。
...
$staticZip = "$publishRoot\daily-atlas-static-v2.6.0-r2-$releaseStamp.zip"
...
5. 确认默认模式下书与电影显示“本地编辑视觉 · 原创主题插画”、没有第三方书封／海报请求，城市显示同源开放许可实图；城市图片故障能恢复本地视觉，本周回顾仍只读本机记录。
...
最后按 [`docs/REAL_DEVICE_MATRIX_v2.6.0.md`](docs/REAL_DEVICE_MATRIX_v2.6.0.md) 用实体设备分别执行 Wi-Fi 与移动数据测试。若电脑能开而某个手机网络在收到任何 HTTP 响应前就超时，应检查该网络的 DNS、代理、运营商与域名可达性；`pages.dev`、`workers.dev` 和自定义域名在不同地区的实际连通性不能由应用代码保证。面向中国大陆稳定公开运营时，还需根据服务器所在地、域名备案和所选云厂商要求另行设计境内部署或镜像。
...
v2.6.0 的主卡、详情占位与搜索结果共用视觉生成器：书影以按稳定 ID 生成的原创本地主题插画作为即时底层，白名单原书封／海报完成解码后覆盖显示；失败、无地址、离线、数据节省或安全模式下继续显示同一条目的插画。城市只使用同源开放许可文件，本地医学图始终保留。紧凑索引仍独立降低首屏目录传输；“完整离线”下载全库详情、搜索索引、500 条朗读和 200 张原发布城市图，不下载第三方书封／海报。移动城市衍生图按需缓存，完整离线时可立即使用已核验原图回退。
...
用户明确选择“完整离线”后，Service Worker 才把 44 个详情分片、延迟搜索索引、500 个朗读和 200 张同源城市图下载到独立暂存 Cache；进度为 `0–700`（音频 `1–500`、城市图 `501–700`）。每项写入前核对字节数和 SHA-256；完整 marker 还会复核真实 Cache 项。暂停、页面刷新、Worker 更新、网络或配额故障会保留已经验证的暂存，继续时只补缺项；“取消”才删除可选暂存。轻量应用壳始终保留。远程书封／电影海报只在联网浏览时渐进显示，不进入应用 Cache 或离线包；实时天气、Web Speech 系统语音包和外部来源页面也不缓存。
...
- 公开 LTS 默认仅联网加载 Open Library 书封，失败时显示原创本地主题插画；不复制进发布包或应用缓存，书目与图片权利边界仍分开披露。
...
IMDb 官方条款把免费数据集限定在个人／非商业用途，并限制把它重制为在线或离线电影信息数据库；因此 v2.6.0 继续将相关数值留在不发布的构建审计输入，不作为公开应用数据。若未来取得明确适用授权，应重新进行许可核对、构建和发布审查。
...
```powershell
npm ci
# v2.6.0 跨引擎门禁需要三套 Playwright 浏览器
...
```powershell
npm run release:preflight
npm run release:package -- --zip "C:\Users\lenovo\Documents\今日万象发布\daily-duet-v2.6.0-r2-YYYYMMDD-HHMMSS.zip"
npm run release:verify -- --zip "C:\Users\lenovo\Documents\今日万象发布\daily-duet-v2.6.0-r2-YYYYMMDD-HHMMSS.zip"
```
