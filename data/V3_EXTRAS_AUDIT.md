...
> 历史快照：本文件冻结 2026-08-25 扩容批次的当时输入、数量与哈希；下文“当前”仅指该批次当时的构建，不能作为 v2.4.x 的现行素材清单。现行候选池口径以 `data/CATALOG_AUDIT.md` 为准，医学图以 `assets/medical/manifest.json` 为准，德语音频以 `assets/audio/german/manifest.json` 为准。

## 结果

- `cities200.json`：200 条；以当时的 `cities70.json` 为前 70 项并扩展 130 项。
- `german500.json`：500 条；以当时的 `german200.json` 为前 200 项并扩展 300 项。
- `medical500.json`：500 条；以当时的 `medical200.json` 为前 200 项并扩展 300 项。
- 当时基线中的来源、安全边界和文字可被后续勘误；“前缀保留”只表示该次构建输入与该次 500 条输出一致，不等于历史版本对象永远不修订。
...
城市卡是静态策展，不替代签证、安全、天气、节庆、交通或开放时间等实时旅行信息；当时版本的天气只在用户主动点击时请求，并显示获取时间。
...
500 条医学卡分布在 12 个主题组，每条均包含一般科普摘要、可执行的日常行动、适用边界或危险信号、风险级别、来源和 `imageTheme`。该次汇总为 415 个不同主题、290 个不同来源 URL；风险分布为一般 153、需谨慎 273、紧急警示 74。
...
医学卡只提供一般健康教育，不根据少量输入诊断或给出个体化治疗。紧急程度、服务可得性和号码随所在地变化，应以当地急救、毒物咨询、专业医疗服务和实际症状为准。当时的六张本地图只承担主题识别，不能替代文字证据或医学判断；现行医学图数量与哈希须读取 `assets/medical/manifest.json`。
