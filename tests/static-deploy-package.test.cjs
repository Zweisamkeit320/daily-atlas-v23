...
test("service-worker and static-deploy manifests share the 2.4 runtime, generated catalog, 24 medical assets, and 200 city visuals", () => {
  ...
  for (const name of ["_headers", "diagnostics.css", "diagnostics.html", "diagnostics.js"]) {
...
test("static deploy ZIP has one root, 24 medical images, 200 city visuals, 500 MP3s, fresh six-pack versions, and a valid SHA-256 sidecar", (t) => {
  ...
  assert.throws(() => Deploy.verifyStaticDeploy(renamed), /ZIP version 2\.1\.0 does not match package version 2\.4\.1/);
...
test("static deploy creation rejects a ZIP version different from package.json and creates no artifact", (t) => {
  ...
  assert.throws(() => Deploy.createStaticDeploy(wrong, fixture.root), /ZIP version 2\.0\.0 does not match package version 2\.4\.1/);
...
test("static deploy source inspection rejects stale catalog, music, and service-worker runtime contracts", (t) => {
  ...
  assert.throws(() => Deploy.inspectSource(fixture.root), /catalog\.js version 2\.0\.0 does not match package version 2\.4\.1/);
